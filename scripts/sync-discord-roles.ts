...
function loadContributionData(): Record<string, any> {
  ...
  const files = fs.readdirSync(contributionsDir).filter((file) =>
    file.endsWith(".json")
  );
...
async function syncRoles(client: Client, guildId: string) {
  ...
  for (const [githubUsername, userData] of Object.entries(contributionData)) {
    ...
    if (!stars || !STAR_TO_ROLE_MAP[stars]) {
      console.warn(`Skipping user ${githubUsername} due to missing or invalid stars.`);
      continue;
    }
    ...
    if (!targetRole) {
      console.warn(`Target role '${targetRoleName}' not found for user ${githubUsername}.`);
      continue;
    }
