...
import { getWeekEndDate, filterByDateRange } from "../utils/date-range-utils"

export async function getAllPRsFixed(
  repo: string,
  since: string,
  ...
  const filteredPRs = prs.filter((pr) => {
    ...
    // PR must be created or merged within the week period
    const createdInRange = createdDate && 
      createdDate >= sinceDate && 
      createdDate <= endDateObj
    
    const mergedInRange = mergedDate && 
      mergedDate >= sinceDate && 
      mergedDate <= endDateObj
