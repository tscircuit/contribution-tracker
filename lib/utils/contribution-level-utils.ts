...
export async function getContributorLevel(username: string): Promise<string> {
  try {
    const contributionOverviewsDir = path.join(process.cwd(), "contribution-overviews")

    // Get all JSON files in the contribution-overviews directory
    const files = fs.readdirSync(contributionOverviewsDir)
    ...
    const data = JSON.parse(fs.readFileSync(filePath, 'utf-8')) as Record<string, ContributorStats>
